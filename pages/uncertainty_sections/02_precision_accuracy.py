...
def render_precision_accuracy_section():
    ...
    if st.button("Check Q1", key="pa_q1_btn"):
        st.success("Correct! Random error refers to a lack of precision.") if q2_1 == "a lack of precision" else st.error("Try again")
    ...
    if st.button("Check Q2", key="pa_q2_btn"):
        st.success("Correct! Systematic error refers to a lack of accuracy.") if q2_2 == "a lack of accuracy" else st.error("Try again")
    ...
    # Q3: Random error is most likely caused by
    ...
    if st.button("Check Q3", key="pa_q3_btn"):
        st.success("Correct! Random error is caused by fluctuations that vary unpredictably from trial to trial.") if q2_3 == "Fluctuations that vary unpredictably from trial to trial" else st.error("Try again")

    # Q4: Systematic errors can possibly be reduced by
    ...
    if st.button("Check Q4", key="pa_q4_btn"):
        st.success("Correct! Systematic errors can be reduced by comparing to a known, more reliable measurement.") if q2_4 == "Comparing to a known, more reliable measurement" else st.error("Try again")

    # Q5: Which type of error is this? (Thermometer thermal contact)
    ...
    if st.button("Check Q5", key="pa_q5_btn"):
        st.success("Correct! This is a systematic error - the same problem affects all measurements.") if q2_5 == "systematic error" else st.error("Try again")

    # Q6: Which type of error is this? (Voltmeter electronic noise)
    ...
    if st.button("Check Q6", key="pa_q6_btn"):
        st.success("Correct! This is a systematic error - the same approximation error affects all calculations.") if q2_6 == "random error" else st.error("Try again")

    # Q7: Which type of error is this? (Mathematical approximation)
    ...
    if st.button("Check Q7", key="pa_q7_btn"):
        st.success("Correct! This is a systematic error - the same approximation error affects all calculations.") if q2_7 == "systematic error" else st.error("Try again")

    # Q8: Why does increasing trials improve the value?
    ...
    if st.button("Check Q8", key="pa_q8_btn"):
        st.success("Correct! Increasing trials reduces random error, not systematic error.") if q2_8 == "Increasing the number of trials will lower the random error." else st.error("Try again")
