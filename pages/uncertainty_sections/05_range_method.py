...
def render_range_method_section():
    st.header("Range method: Estimating uncertainty with the range of multiple measurements")
    ...
    st.markdown("**Should you use the range method in your physics lab if you have 10 trials?**")
    q5_1 = st.radio(
        "Select one",
    ...
    if st.button("Check Q1", key="range_q1_btn"):
        st.success("Correct! With 10 trials, you should use a different method.") if q5_1 == "No, use a different method." else st.error("Try again")
    ...
    st.markdown("**Here are measurements from 5 trials:**")
    ...
    if st.button("Check Q2", key="range_q2_btn"):
        st.success("Correct! 8.1 ± 0.5 V") if q5_2 == "8.1 ± 0.5 V" else st.error("Try again")
